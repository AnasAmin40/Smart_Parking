...
const UpcomingBookings = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:5266/api/Booking/GetUpcomingBooking/${user.userID}`
      )
      .then((response) => {
  ...
  return (
    <>
      <div className="content-area" id="contentArea">
        <h3 className="mb-4">Upcoming Booking Status</h3>
        <div className="row g-3">
          {UpcomingBookings.length > 0 ? (
            UpcomingBookings.map((curElm) => (
            ...
            <div className="text-center text-muted p-5">
              <h5>❌ No Upcoming Booking found.</h5>
            </div>
          )}
        </div>
      </div>
